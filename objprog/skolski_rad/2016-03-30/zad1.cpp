...
void Igra::unosPozicije(void)
{
    ...
    while(znakNaPoziciji == 'X' || znakNaPoziciji == 'X');

    mapa[pozicija / 3][pozicija % 3 - 1] = 'X';
}
...
void Igra::provera(void)
{
    int i;
    for(i = 0; i < 3; i++)
    {
        if(mapa[i][0] == mapa[i][1] == mapa[i][2])
        {
            aktivna = false;
            return;
        }
        if(mapa[0][i] == mapa[1][i] == mapa[2][i])
        {
            aktivna = false;
            return;
        }
    }
    if(mapa[0][0] == mapa[1][1] == mapa[2][2])
        aktivna = false;
    else if(mapa[0][2] == mapa[1][1] == mapa[2][0])
        aktivna = false;
}
        ...
        int *tip, int *pozicija)
{
    ...
    */
    ...
    if(mapa[x1][y1] == 'O')
    {
        if(mapa[x1][y1] == mapa[x2][y2])                        // Najveci prioritet
        {
            if(mapa[x3][y3] != 'X')
            {
                *pozicija = x3 * 3 + y3 + 1;
        ...
        } else if(mapa[x1][y1] != mapa[x2][y2])
        {
            if(mapa[x2][y2] != 'X' && mapa[x3][y3] != 'X')
            {
                if(*tip > 2)
                {
                    *tip = 2;
    ...
    } else if(mapa[x1][y1] == 'X')
    {
        if(mapa[x1][y1] == mapa[x2][y2])                        // 2 najveci prioritet
        {
            if(mapa[x3][y3] != 'X')
            {
                *tip = 1;
